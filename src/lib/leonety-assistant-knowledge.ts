...
export function buildLeonetyAssistantKnowledge(locale: Locale, pathname: string) {
  return JSON.stringify({
    ...
    features: {
      ...
      notifications: 'System notifications are enabled per browser/device in Settings > Notifications after explicit permission. iPhone/iPad background push requires the installed Home Screen web app. The operating system controls sound, volume, Focus and Do Not Disturb. WooCommerce incoming-order delivery still selects one alert device per workspace in its integration settings.',
    ...
    commonTasks: [
      ...
      'Enable system notifications: open Settings > Notifications, choose Enable notifications and grant browser permission. Configure the selected WooCommerce order-alert device separately in Settings > Integrations > WooCommerce.',
