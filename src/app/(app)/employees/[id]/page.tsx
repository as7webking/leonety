  ...
  return (
    <PageContainer>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        {section(t('employees.profile.personal'), <><Value label={t('employees.profile.firstName')} value={employee.first_name} /><Value label={t('employees.profile.lastName')} value={employee.last_name} /><Value label={t('employees.email')} value={employee.email} /><Value label={t('employees.phone')} value={employee.phone} /><Value label={t('employees.profile.birthDate')} value={date(employee.birth_date)} /><Value label={t('employees.profile.birthPlace')} value={employee.birth_place} /><Value label={t('employees.profile.birthCountry')} value={employee.birth_country} /><Value label={t('employees.profile.nationality')} value={employee.nationality} /></>)}
        {section(t('employees.profile.address'), <><Value label={t('employees.profile.street')} value={[employee.street, employee.house_number].filter(Boolean).join(' ')} /><Value label={t('employees.profile.city')} value={[employee.postal_code, employee.city].filter(Boolean).join(' ')} /></>)}
        {section(t('employees.profile.taxSocial'), <><Value label={t('employees.profile.taxId')} value={employee.tax_id} /><Value label={t('employees.profile.taxClass')} value={employee.tax_class} /><Value label={t('employees.profile.socialSecurityNumber')} value={employee.social_security_number} /><Value label={t('employees.profile.healthInsurance')} value={employee.health_insurance_provider} /></>)}
