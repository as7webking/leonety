...
function ClientRow({ client, metric, locale, fallbackCurrency, t, view, archive, restore }: { client: ClientRecord; metric?: ClientMetrics; locale: string; fallbackCurrency: string; t: (key: string) => string; view: ClientView; archive: (client: ClientRecord) => Promise<void>; restore: (client: ClientRecord) => Promise<void> }) {
  ...
  return <tr><td className="px-4 py-4"><Link href={`/app/clients/${client.id}`} className="font-medium text-slate-950 hover:text-blue-700">{client.client_company || client.name}</Link>{client.client_company && <p className="mt-1 text-xs text-slate-500">{client.name}</p>}</td><td className="max-w-52 px-4 py-4 text-slate-600"><p className="truncate">{client.email || '—'}</p><p>{client.phone || '—'}</p></td><td className="px-4 py-4">{invoiceCount > 0 ? <Link href={buildClientInvoicesHref(client.id)} className="font-medium text-blue-700 hover:underline" aria-label={`${t('clients.crm.invoices')}: ${client.client_company || client.name}`}>{invoiceCount}</Link> : 0}</td><td className="px-4 py-4 font-medium text-amber-700">{formatCurrencyGroups(metric?.unpaid ?? {}, locale, fallbackCurrency)}</td><td className="px-4 py-4 font-medium text-emerald-700">{formatCurrencyGroups(metric?.paid ?? {}, locale, fallbackCurrency)}</td><td className="px-4 py-4">{metric?.contractCount ?? 0}</td><td className="px-4 py-4 text-slate-600">{metric?.lastActivity ? new Intl.DateTimeFormat(locale).format(new Date(metric.lastActivity)) : '—'}</td><td className="px-4 py-4"><RowActions client={client} t={t} view={view} archive={archive} restore={restore} /></td></tr>
}
